...
def reinit_analysis(
    ...
    if "base_cases_simulation_data_df" in st.session_state:
        del st.session_state["base_cases_simulation_data_df"]
    if "calibrated_simulations_data_df" in st.session_state:
        del st.session_state["calibrated_simulations_data_df"]
    if "custom_simulations_data_df" in st.session_state:
        del st.session_state["custom_simulations_data_df"]
...
def create_dynawo_tab():
    ...
    def on_click_run_all():
        ...
        # -----------------------------------------------------------------
        st.session_state["base_cases_simulation_data_df"] = simulation_data
        st.session_state["base_cases_correlation_dicts"] = correlations
...
#         base_cases_correlation_dict = st.session_state["base_cases_correlation_dict"] # {case_1 : correlation_dict, case_2 : ..., ...}
#         similarity_p_df, similarity_q_df, similarity_pq_df \
#             = create_correlation_df(base_cases_correlation_dict, "base_case vs measures")
...
def create_plots():
    ...
    if not are_measures_uploaded():
        st.write("You need to upload the measured data first")
        return

    measured_dic = st.session_state["measured_data_df_dic"]
    base_dic = st.session_state.get("base_cases_simulation_data_df", {})
    calib_dic = st.session_state.get("calibrated_simulations_data_df", {})
    custom_dic = st.session_state.get("custom_simulations_data_df", {})

    # Liste des cases
    cases = measured_dic.keys()
    ...
    for case_name in cases:
        ...
        measured_df = measured_dic[case_name]
        base_df = base_dic.get(case_name)
        calib_df = calib_dic.get(case_name)
        custom_df = custom_dic.get(case_name)
        ...
        if base_df is not None:
            fig_p.add_trace(go.Scatter(
                x=base_df.index,
                y=base_df[col_name_p],
                mode="lines",
                name="P_base_case",
                line=dict(color="blue", width=1.5)
            ))

        # Automatic calibration (green)
        if calib_df is not None:
            fig_p.add_trace(go.Scatter(
                x=calib_df.index,
                y=calib_df[col_name_p],
                mode="lines",
                name="P_auto_calibration",
                line=dict(color="green", width=1.5)
            ))

        # Custom calibration (cyan)
        if custom_df is not None:
            fig_p.add_trace(go.Scatter(
                x=custom_df.index,
                y=custom_df[col_name_p],
                mode="lines",
        ...
        fig_p.update_layout(
            title=f"P curves for {case_name}",
            xaxis_title="Time",
            yaxis_title=col_name_p
        )
        ...
        if calib_df is not None:
            fig_q.add_trace(go.Scatter(
                x=calib_df.index,
                y=calib_df[col_name_q],
                mode="lines",
                name="Q_auto_calibration",
                line=dict(color="green", width=1.5)
            ))
        ...
        fig_q.update_layout(
            title=f"Q curves for {case_name}",
            xaxis_title="Time",
            yaxis_title=col_name_q
        )
...
def create_plot_tab():
    #create_correlation_panel() #to be adapted
    create_plots()

#to be adapted to several cases
        ...
        with col_3:
            ...
            def on_click():
                ...
                if len(selected_sets) > 0:
                    ...
                    sampled_measured_p = sampled_measured_data_df[col_name_p].values
                    sampled_measured_q = sampled_measured_data_df[col_name_q].values
                    base_case_rmse = st.session_state["base_cases_correlation_dict"]["rmse"]
...
def create_param_calibration_tab():
    if is_base_case_calculated():
        ...
        with col_3:
            ...
            def on_click():
                ...
                if "calibration_errors" not in st.session_state:
                    st.session_state["calibration_errors"] = ""
                ...
                if len(selected_sets) > 0:
                    ...
                    temp_folders_calibration_cases = st.session_state["temp_folders_calibration_cases"]
                    cases = temp_folders_calibration_cases.keys()
                    jobs_files_calibration = {}
                    par_files_calibration = {}
                    ...
                    for case_name in cases:
                        for element in os.listdir(st.session_state["temp_folders_base_cases"][case_name]):
                            src = os.path.join(st.session_state["temp_folders_base_cases"][case_name], element)
                            if os.path.isfile(src):
                                shutil.copy(src, temp_folders_calibration_cases[case_name])
                    
                        jobs_files_calibration[case_name] = os.path.join(
                            temp_folders_calibration_cases[case_name],
                            os.path.basename(st.session_state["jobs_files"][case_name])
                        )
                        par_files_calibration[case_name] = os.path.join(
                            temp_folders_calibration_cases[case_name],
                            os.path.basename(st.session_state["par_files"][case_name])
                        )

                        col_name_p, col_name_q = get_col_name_p_q()
                        sampled_measured_p_dic[case_name] = sampled_measured_data_df_dic[case_name][col_name_p].values
                        sampled_measured_q_dic[case_name] = sampled_measured_data_df_dic[case_name][col_name_q].values
                        
                        base_cases_rmse[case_name] = st.session_state["base_cases_correlation_dicts"][case_name]["rmse"]

                    dynawo_launcher = st.session_state["dynawo_launcher"]

                    try:
                        calibrated_simulations_data_df = run_parameter_calibration(
                            dynawo_launcher,
                            jobs_files_calibration, #path to jobs files paths
                            par_files_calibration, #path to par files paths
                            selected_sets, #parameters selected
                            sampled_measured_p_dic, #p column for each case
                            sampled_measured_q_dic, #q column for each case
                            base_cases_rmse, #rmse for each base case
                            optim_method,
                            streamlit_logger=streamlit_logger_calibration
                        )

                        print('---- OUT parameter calibration function -----')
                        sampled_calibrated_simulations_data_df = {}
                        calibrated_correlation_dicts = {}
                        sampled_measured_data_df_dic = st.session_state["sampled_measured_data_df_dic"]
                        
                        for case_name, sampled_measured_data_df in sampled_measured_data_df_dic.items():
                            ...
                            sampled_calibrated_simulations_data_df[case_name] = sample_df(
                                calibrated_simulations_data_df[case_name],
                                measured_data_start_time,
                                measured_data_end_time
                            )
                            print('----- SAMPLED -----')

                            calibrated_correlation_dicts[case_name] = create_correlation_dict(
                                sampled_calibrated_simulations_data_df[case_name], 
                                sampled_measured_data_df_dic[case_name]
                            )

                        st.session_state["calibrated_simulations_data_df"] = calibrated_simulations_data_df
                        st.session_state["calibrated_correlation_dicts"] = calibrated_correlation_dicts
                        st.session_state["log_area_base_case_calibration_content"] = get_streamlit_logs(streamlit_logger_calibration)
                        
                        print('----------------DONNEES CALIBRATION ENREGISTREES-------------')

                    except Exception as e:

                        # Nettoyage éventuel
                        st.session_state.pop("calibrated_simulations_data_df", None)
                        st.session_state.pop("calibrated_correlation_dicts", None)

                        # Ajout au log persistent
                        if "calibration_errors" not in st.session_state:
                            st.session_state["calibration_errors"] = ""
                        st.session_state["calibration_errors"] += f"\n\n❌ {type(e).__name__}: {str(e)}\n"

                        # Affichage visible qui NE DISPARAÎT PAS
                        st.error("Une erreur est survenue pendant la calibration :")
                        st.exception(e)

                        st.stop()  # Empêche le rerun automatique
    ...
    if "calibration_errors" in st.session_state:
        st.subheader("Console erreurs calibration")
        st.code(st.session_state["calibration_errors"])
    ...
    if is_base_case_calculated():
        ...
        with col_3:
            ...
            def on_click():
                ...
                if len(selected_sets) > 0:
                    temp_folders_custom_calibration_cases = st.session_state["temp_folders_custom_calibration_cases"]
                    cases = temp_folders_custom_calibration_cases.keys()
                    jobs_files_calibration = {}
                    par_files_calibration = {}
                    ...
                    for case_name in cases:
                        for element in os.listdir(st.session_state["temp_folders_base_cases"][case_name]):
                            src = os.path.join(st.session_state["temp_folders_base_cases"][case_name], element)
                            if os.path.isfile(src):
                                shutil.copy(src, temp_folders_custom_calibration_cases[case_name])

                        jobs_files_calibration[case_name] = os.path.join(
                            temp_folders_custom_calibration_cases[case_name],
                            os.path.basename(st.session_state["jobs_files"][case_name])
                        )
                        par_files_calibration[case_name] = os.path.join(
                            temp_folders_custom_calibration_cases[case_name],
                    ...
                    try:
                        ...
                        custom_simulations_data_df = {}
                        sampled_custom_simulations_data_df = {}
                        custom_correlation_dicts = {}
                        sampled_measured_data_df_dic = st.session_state["sampled_measured_data_df_dic"]
                        
                        for case_name, sampled_measured_data_df in sampled_measured_data_df_dic.items(): 
                            custom_simulations_data_df[case_name] = run_custom_dynawo(
                                dynawo_launcher,
                                jobs_files_calibration[case_name],
                                par_files_calibration[case_name],
                                selected_sets,
                            ...
                            measured_data_start_time = sampled_measured_data_df.index[0]
                            measured_data_end_time = sampled_measured_data_df.index[-1]
                            sampled_custom_simulations_data_df[case_name] = sample_df(
                                custom_simulations_data_df[case_name],
                                measured_data_start_time,
                                measured_data_end_time
                            )
                            print('----- SAMPLED -----')

                            custom_correlation_dicts[case_name] = create_correlation_dict(
                                sampled_custom_simulations_data_df[case_name],
                                sampled_measured_data_df_dic[case_name]
                                )

                        st.session_state["custom_simulations_data_df"] = custom_simulations_data_df
                        st.session_state["custom_correlation_dicts"] = custom_correlation_dicts
                        st.session_state["log_area_custom_calibration_content"] = get_streamlit_logs(streamlit_logger_custom_calibration)

                    except DynawoFailedException:
                        if "custom_simulations_data_df" in st.session_state:
                            del st.session_state["custom_simulations_data_df"]
                        if "custom_correlation_dicts" in st.session_state:
                            del st.session_state["custom_correlation_dicts"]
